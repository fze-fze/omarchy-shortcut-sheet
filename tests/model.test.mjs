...
test("shows complete truncated shortcuts in a hover overlay", async () => {
  ...
  assert.match(source, /root\.showShortcutHover\(rowRoot,\s*rowRoot\.modelData,\s*rowRoot\.flatIndex\)/)
  assert.match(source, /id:\s*expandedShortcut/)
  assert.match(source, /width:\s*Math\.max\(root\.shortcutHoverRowWidth,\s*preferredWidth\)/)
  assert.match(source, /visible:\s*root\.shortcutHoverVisible\s*&&\s*!root\.settingsOpen/)
  ...
  assert.match(source, /function showSelectedShortcutHover\(\)/)
  assert.match(source, /itemList\.itemAtIndex\(localIndex\)/)
  assert.match(source, /onContentYChanged:[\s\S]*?Qt\.callLater\(columnRoot\.showSelectedShortcutHover\)/)
  assert.match(source, /ensureSelectionVisible\(\)[\s\S]*?Qt\.callLater\(columnRoot\.showSelectedShortcutHover\)/)
})
